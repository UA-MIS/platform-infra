...
| **Platform Health** | `dashboard-platform-health.yaml` | ArgoCD Synced/Healthy counts + apps needing attention (real); Vault/Harbor/Crossplane/ARC (**proxies** — pod/deployment/statefulset status via kube-state-metrics, not each component's own `/metrics`) | ArgoCD ServiceMonitor (real) + kube-state-metrics (proxy for the rest) |
...
- **Vault true seal status** — `VaultSealedOrDown` and the Platform Health panel both
  ...
  `telemetry` stanza enabled + a ServiceMonitor.
- **Harbor** — no ServiceMonitor exists; Harbor core exposes `/metrics` when
  `metric.enabled` is set in the Helm values. The Platform Health dashboard proxies
  with `kube_deployment_status_replicas_ready`/`_spec_replicas` in the `harbor` ns.
- **Crossplane provider + composition health** — Crossplane's own controller-runtime
  `/metrics` isn't scraped; no ServiceMonitor exists for `crossplane-system`. The XR/
  Composition `status.healthy` field also isn't a Prometheus metric without kube-
  state-metrics custom-resource-state config. Proxied today via pod
  running-count/restarts in `crossplane-system`.
- **ARC runner scale-set activity (queued/running/per-team)** — the
  `gha-runner-scale-set-controller` + listener expose their own metrics; nothing
  scrapes `arc-system` today. Runner/build pods also carry no Prometheus-visible
  label for their owning team (`actions.github.com/scale-set-name` would need
