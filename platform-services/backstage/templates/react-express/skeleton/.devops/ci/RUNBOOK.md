...
- `docker`, `git`, `yq`, `node` + `npm` on PATH.
...
$EDITOR backend/src/index.ts frontend/src/App.tsx
( cd backend && npm run typecheck ) && ( cd frontend && npm run typecheck )   # keep it green
