// Database access for the Nitro server — Drizzle ORM over a mysql2 connection pool,
// built from DATABASE_URL (a standard mysql:// URI, e.g.
// `mysql://user:pass@host:3306/dbname`). The platform chart injects DATABASE_URL from the
// per-env app-secret (External Secrets Operator + Vault) — set it via The Process
// "Secrets" tab under the key DATABASE_URL. NEVER hardcode credentials.
//
...
// green on a fresh repo with nothing in Vault; the data routes return a clear 503.
