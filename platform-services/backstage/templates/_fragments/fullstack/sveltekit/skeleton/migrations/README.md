...
For convenience the app creates the sample `items` table idempotently on the first request
(`src/lib/server/db.ts` -> `ensureSchema()`, called from `src/hooks.server.ts`), so the page
...
The data layer uses **Drizzle ORM** (`src/lib/server/schema.ts`). For real projects manage
...
When you adopt this, remove the `ensureSchema()` call from `src/hooks.server.ts` and manage
all schema changes through the generated files (run `db:migrate` as a deploy step / job,
not on every pod start). Prefer Prisma or Knex instead? Either works the same way — swap the
data layer in `src/lib/server/db.ts` and keep reading `DATABASE_URL`.
