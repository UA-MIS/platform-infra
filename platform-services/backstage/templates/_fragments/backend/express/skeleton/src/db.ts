...
//
// Zero-config friendly (mirrors the platform's optional-secret pattern): if DATABASE_URL
// is unset the pool is null and the data routes return 503 with a clear message, while
// /healthz stays green so the pod still becomes Ready on a fresh repo with nothing in Vault.
