...
export default defineConfig({
  ...
  server: {
    port: 5173,
    proxy: {
      // LOCAL DEV ONLY: forward /api to the Express backend running on the same machine,
      // so the browser can call a relative `/api/...` URL exactly like it does in prod.
      // In production there is NO proxy — the platform ingress routes /api -> the backend
