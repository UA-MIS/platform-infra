...
2. Run the tests: `cd backend && go test ./...` and `cd frontend && go test ./...`.
...
bootstrap block shipped (commented) in your component's Dockerfile (e.g. `frontend/Dockerfile`). Two platform facts cause it:
...
Copy the verbatim block from the bottom of your component's Dockerfile (e.g. `frontend/Dockerfile`) into any Debian-base stage
