...
# stopped looking", and the second must fail. These are the two twins every tenant's CI
# is built from, plus the promotion path.
REQUIRED = [
...
def main() -> int:
    ...
    print(f"  parsed {n_args} docker-action `args:` block(s) and {n_run} `run:` block(s) "
          f"across {len(REQUIRED)} workflow file(s)")
    if n_args == 0:
