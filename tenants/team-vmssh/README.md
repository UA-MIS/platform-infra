...
| File | Purpose |
| --- | --- |
| `vm/appproject-vm.yaml` | the VM-tier tenancy fence — a SEPARATE AppProject `vmssh-vm` whitelisting the VM kinds + the cloud-init `Secret`; destinations only `vmssh-vm-*`; `clusterResourceWhitelist: []`. |
| `vm/applicationset-vm.yaml` | the VM env ApplicationSet — single-env (prod) App that syncs the app repo's VM chart into `vmssh-vm-prod`. **The piece that actually deploys the VM.** |
| `vm/namespaces/vm-prod.yaml` | `vmssh-vm-prod` Namespace at PSA `baseline` + VM-sized quota/limitrange + 4 NetworkPolicies + VM-aware Role/RoleBinding (console/VNC + power). |
...
Replace three tokens everywhere — `vmssh` (team slug, a DNS label), `vmssh`
(the app repo name — `UA-MIS/<appName>`, NOT `<team>-app`), and `2026-summer`
(cohort, e.g. `2026-fall`). Substitute `vmssh` BEFORE `vmssh` so a
`vmssh`-prefixed appName isn't half-replaced:

```bash
TEAM=acme APPNAME=acme SEMESTER=2026-fall
cp -r tenants/_template-vm tenants/team-$TEAM
grep -rl 'vmssh\|vmssh\|2026-summer' tenants/team-$TEAM \
  | xargs sed -i "s/vmssh/$APPNAME/g; s/vmssh/$TEAM/g; s/2026-summer/$SEMESTER/g"
...
| --- | --- | --- |
| `vmssh` | team slug — the VM AppProject name + `vmssh-vm-*` namespace prefix + OIDC/RBAC group `vmssh-developers` (D-026) | `acme` |
| `vmssh` | the app repo name — `UA-MIS/<appName>` (repo == appName, #101); the VM ApplicationSet + AppProject source point here | `acme` |
