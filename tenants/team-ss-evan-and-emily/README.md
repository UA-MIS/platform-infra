...
| File | Purpose |
| --- | --- |
| `vm/appproject-vm.yaml` | the VM-tier tenancy fence — a SEPARATE AppProject `ss-evan-and-emily-vm` whitelisting the VM kinds + the cloud-init `Secret`; destinations only `ss-evan-and-emily-vm-*`; `clusterResourceWhitelist: []`. |
| `vm/applicationset-vm.yaml` | the VM env ApplicationSet — single-env (prod) App that syncs the app repo's VM chart into `ss-evan-and-emily-vm-prod`. **The piece that actually deploys the VM.** |
| `vm/namespaces/vm-prod.yaml` | `ss-evan-and-emily-vm-prod` Namespace at PSA `baseline` + VM-sized quota/limitrange + 4 NetworkPolicies + VM-aware Role/RoleBinding (console/VNC + power). |
...
Replace three tokens everywhere — `ss-evan-and-emily` (team slug, a DNS label), `ss-evan-and-emily`
(the app repo name — `UA-MIS/<appName>`, NOT `<team>-app`), and `2026-fall`
(cohort, e.g. `2026-fall`). Substitute `ss-evan-and-emily` BEFORE `ss-evan-and-emily` so a
`ss-evan-and-emily`-prefixed appName isn't half-replaced:

```bash
TEAM=acme APPNAME=acme SEMESTER=2026-fall
cp -r tenants/_template-vm tenants/team-$TEAM
grep -rl 'ss-evan-and-emily\|ss-evan-and-emily\|2026-fall' tenants/team-$TEAM \
  | xargs sed -i "s/ss-evan-and-emily/$APPNAME/g; s/ss-evan-and-emily/$TEAM/g; s/2026-fall/$SEMESTER/g"
...
| --- | --- | --- |
| `ss-evan-and-emily` | team slug — the VM AppProject name + `ss-evan-and-emily-vm-*` namespace prefix + OIDC/RBAC group `ss-evan-and-emily-developers` (D-026) | `acme` |
| `ss-evan-and-emily` | the app repo name — `UA-MIS/<appName>` (repo == appName, #101); the VM ApplicationSet + AppProject source point here | `acme` |
