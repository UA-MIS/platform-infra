...
| File | Purpose |
| --- | --- |
| `appproject-vm.yaml` | the VM-tier tenancy fence — a SEPARATE AppProject `demovm-vm` that whitelists `VirtualMachine`/`VirtualMachineInstance`/`DataVolume` (+ the cloud-init `Secret`) and targets only `demovm-vm-*` namespaces. `clusterResourceWhitelist: []`. |
| `applicationset-vm.yaml` | the VM env ApplicationSet — a single-env (prod) `matrix(list × git-files promotion.yaml)` App that syncs the APP repo's `.devops/chart/overlays/prod` VM chart into `demovm-vm-prod` under project `demovm-vm`. **Without this the VM tier is a fence + namespace with nothing inside it** (the #376 onboarding bug). The VM analogue of `../_template/applicationset-envs.yaml`. |
| `namespaces/vm-prod.yaml` | `demovm-vm-prod` Namespace at **PSA `baseline`** (not restricted) + VM-sized ResourceQuota + LimitRange + 4 NetworkPolicies (default-deny, Traefik ingress, DNS egress, importer image-pull) + VM-aware Role/RoleBinding. |
...
- A team **cannot** create a VM in their `restricted` container namespaces — the VM
  kinds are whitelisted only in the `demovm-vm` project (GitOps layer) and
...
and `s/prod/dev/`; the AppProject destination is a `demovm-vm-*` wildcard, so no
...
Same as the parent template: `demovm`, `demovm`, `2026-summer`. Substitute
`demovm` BEFORE `demovm` (a `demovm`-prefixed appName would otherwise be
half-replaced). The team group `demovm-developers` is the same subject used by the
container tier.
