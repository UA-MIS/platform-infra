...
| File | Purpose |
| --- | --- |
| `vm/appproject-vm.yaml` | the VM-tier tenancy fence — a SEPARATE AppProject `paper-papas-vm` whitelisting the VM kinds + the cloud-init `Secret`; destinations only `paper-papas-vm-*`; `clusterResourceWhitelist: []`. |
| `vm/applicationset-vm.yaml` | the VM env ApplicationSet — single-env (prod) App that syncs the app repo's VM chart into `paper-papas-vm-prod`. **The piece that actually deploys the VM.** |
| `vm/namespaces/vm-prod.yaml` | `paper-papas-vm-prod` Namespace at PSA `baseline` + VM-sized quota/limitrange + 4 NetworkPolicies + VM-aware Role/RoleBinding (console/VNC + power). |
...
Replace three tokens everywhere — `paper-papas` (team slug, a DNS label), `paper-papas`
(the app repo name — `UA-MIS/<appName>`, NOT `<team>-app`), and `2026-fall`
(cohort, e.g. `2026-fall`). Substitute `paper-papas` BEFORE `paper-papas` so a
`paper-papas`-prefixed appName isn't half-replaced:

```bash
TEAM=acme APPNAME=acme SEMESTER=2026-fall
cp -r tenants/_template-vm tenants/team-$TEAM
grep -rl 'paper-papas\|paper-papas\|2026-fall' tenants/team-$TEAM \
  | xargs sed -i "s/paper-papas/$APPNAME/g; s/paper-papas/$TEAM/g; s/2026-fall/$SEMESTER/g"
...
| --- | --- | --- |
| `paper-papas` | team slug — the VM AppProject name + `paper-papas-vm-*` namespace prefix + OIDC/RBAC group `paper-papas-developers` (D-026) | `acme` |
| `paper-papas` | the app repo name — `UA-MIS/<appName>` (repo == appName, #101); the VM ApplicationSet + AppProject source point here | `acme` |
